...
@app.route("/vibe-rater/", methods=["GET"])
def root():
    return {"message": "welcome to my vibe rater"}


@app.route("/vibe-rater/chat/", methods=["POST"])
def add_chat():
    chat_rating = vibe_rater(request.json["message"])
    return chat_rating


@app.route("/vibe-rater/serial/", methods=["GET"])
def run_bubbles():
    with serial.Serial(
